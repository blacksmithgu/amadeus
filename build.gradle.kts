...
tasks {
    ...
    val jvmJar by existing(Jar::class) {
        ...
        from(File(webpackTask.get().destinationDirectory, webpackTask.get().outputFileName!!)) // bring output file along into the JAR
    }
    ...
    val run by existing(JavaExec::class) {
        classpath(named<Jar>("jvmJar")) // so that the JS artifacts generated by `jvmJar` can be found and served
    }
}
